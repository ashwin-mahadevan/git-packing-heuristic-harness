...
static intmax_t strategy_stat_proposed;
				     ...
				     unsigned nr)
{
	...
	for (i = 0; i < nr; i++) {
		...
		p->child->depth = p->parent->depth + 1;
		strategy_stat_accepted++;
	}
}
					...
					unsigned n)
{
	...
	trace2_data_intmax("pack-objects", the_repository,
			   "delta-strategy/proposed", strategy_stat_proposed);
	trace2_data_intmax("pack-objects", the_repository,
